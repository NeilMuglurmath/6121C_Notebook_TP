		...
		\newpage
			...
			\newpage
				...
				\begin{itemize}
					...
					\item Puncher/Catapult
					\begin{itemize}
						\item Along with the flywheel, punchers and catapults are also widely used to launch balls in VRC. They usually work by having a motor spin a slip gear, which is just a normal gear but with a portion of the teeth shaved off. The slipgear spins and pulls a mechanism back, and when the gear runs out of teeth (the shaved portion is in contact with the mechanism its pulling back), the mechanism launches, commonly with the help of potential energy stored in rubber bands.
						\begin{figure}[H]
						...
						\item Punchers are popular because of their quick windup. Unlike a flywheel, which has to accelerate to a certain velocity, a puncher only needs to wind back and release. Also, a puncher is extremely consistent; it is built for shooting a certain distance and a certain distance only.

						\item One problem with punchers is that the rubber bands need to be re-tensioned once in a while. This is because after repeated use, the rubber bands get looser, which result in them not storing as much energy as it used to.
					\end{itemize}

					\paragraph{Verdict: Flywheel}
					After considering the positives and negatives of both established launcher designs, we decided on the flywheel. We felt that to have a strong robot, it needs to be able to score flags from more than one position. Also, we did not want to deal with re-tensioning our launcher.
				\end{itemize}

				%ball intake
				\subsubsection{Ball intake}
				\begin{itemize}
					\item Top Roller and Chain
					\begin{itemize}
						\item The combination of a top roller and chain to intake a ball was a popular intake option during the VRC 2015-16 game, Nothing But Net. It consists of a top roller in the front of the intake which pulls balls into the second top roller. Then, the balls are pulled up to the flywheel via chain, and right before the flywheel, some teams might decide to use an accelerator, which propels the balls toward the flywheel. The accelerator is very useful because it stops balls from accidentally being fired and it can allow the driver to freely use the intake after intaking one ball. The problem with this is that it requires a motor.
						\begin{figure}[H]
